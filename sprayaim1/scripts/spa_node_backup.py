...
pump_pub = rospy.Publisher('pump_status_topic', String, queue_size=10)
gimbal_pub = rospy.Publisher('gimbal_status_topic', String, queue_size=10)
iaft_pub = rospy.Publisher('ia_firetarget_topic', iafiretarget, queue_size=10)
iafp_pub = rospy.Publisher('ia_pumpstatus_topic', iapumpstatus, queue_size=10)
iawm_pub = rospy.Publisher('ia_waterstatus_topic', iawaterstatus, queue_size=10)

def water_monitor_Callback(data):
    """
    Callback function for receiving water level status.
    Updates the water level status based on received data.
    """
    # rospy.loginfo("Received WaterStatus message: %s", data)
...
def spray_server_Callback(goal):
    ...
    global current_state, water_level_status
     ...
     # Print received goal information
    ...
    iapumpstatus_msg.sequence_id = goal.pump_control.header.seq  # Assuming header is part of pump_control
    ...
    # If data indicates "off" and pump is ON or water level is Low, turn OFF the pump
        ...
        result.done = pump_control(False)  # Stop the pump (False)
        if result.done:
...
def spa_node():
    ...
    rospy.Subscriber('water_monitor_node/water_status_topic', WaterStatus, water_monitor_Callback)
