...
{
  static float battery_safety_threshold = 30.0;
  battery_percentage = battery_safety_threshold;
...
void battery_status(const std_msgs::String::ConstPtr& msg) {                              //Battery Status Callback function
      ROS_INFO ("%s: '%s'\n", __func__, msg->data.c_str());
        ...
        if (std::string(msg->data.c_str()).compare("SafetyCritical") == 0){ 
        Verdict="Pass";
        ROS_INFO ("Verdict: %s\n", Verdict.c_str());
        }
        else{
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "test_case_4i");
  ros::NodeHandle n;
